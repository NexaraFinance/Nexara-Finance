...
try:
    # Inicialización nativa oficial recomendada por Google
    client = genai.Client()
...
with tab1:
    ...
    with col_em1:
        ...
        if st.button("Generar Borrador con Gemini", key="btn_generar_borrador"):
    ...
    with col_em2:
        if 'borrador_correo' in st.session_state:
            ...
            if st.button("🚀 ENVIAR CORREO REAL AHORA VIA GMAIL API", key="btn_enviar_gmail_real"):
                if destinatario:
...
with tab2:
    ...
    canal = st.selectbox("Selecciona el Canal Objetivo:", ["LinkedIn Corporativo", "Newsletter para Pymes", "Script para Video Corto (Reels/TikTok)"], key="select_canal_mkt")
    audiencia_objetivo = st.text_input("Sector de la Pyme objetivo:", value="Empresas de distribución y logística, comercio minorista", key="input_aud_mkt")
    
    if st.button("Estructurar Estrategia de Contenido", key="btn_generar_mkt"):
        with st.spinner("Analizando pilares de comunicación de Nexara..."):
            ...
            response_mkt = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction=NEXARA_CONTEXT + " Regla estricta: Termina siempre el post con una CTA o pregunta sumamente clara enfocada a agendar una auditoría preventiva, tal como exige el manual de marca.",
                    temperature=0.7
                )
            )
    ...
    if st.button("Procesar Acta de Reunión", key="btn_procesar_acta"):
        if notas_caoticas:
            with st.spinner("Estructurando puntos de acción..."):
                ...
                st.markdown("### 📅 Plan de Acción y Siguientes Steps")
